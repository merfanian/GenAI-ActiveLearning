...
from utils.attribute_mappings import FRUITS_LABEL_MAPPING, FRUITS_LABEL_TARGET, ADIENCE_GENDER_MAPPING, ADIENCE_GENDER_TARGET
# --- Configuration ---
RESULTS_DIR = Path("/home/mahdi/Projects/GenAI-ActiveLearning/experiments/ablation_2.2_ccds_sampling/adience_optimized_results")
...
ARCHITECTURE = "resnet"
NUM_ITERATIONS = 10  # More iterations to see clear differences
AUGMENTATION_BATCH_SIZE = 10  # Larger batch size for better improvements
NUM_RUNS = 2  # More runs for statistical significance

ATTRIBUTE_MAPPING = ADIENCE_GENDER_MAPPING
TARGET_LABEL_MAPPING = ADIENCE_GENDER_TARGET

def run_scenario(scenario_name, params, run_number):
    # Configure logging for each specific run
    log_file = RESULTS_DIR / f"{scenario_name}_run_{run_number}.log"
    logging.basicConfig(level=logging.INFO,
                        format='%(asctime)s - %(name)s - %(levelname)s - %(message)s',
                        handlers=[logging.FileHandler(log_file, mode='w'), logging.StreamHandler()])
    
    logging.info(f"--- Running Scenario: {scenario_name}, Run: {run_number} ---")
    
    # Create a unique directory for this scenario run
    scenario_augmented_dir = Path(f"augmented_images_{scenario_name}_run_{run_number}")
    ...
    scenario_model_path = TRAINED_MODELS_DIR / f"{scenario_name}_run_{run_number}_model.pth"
    ...
    for i in range(NUM_ITERATIONS):
        ...
        generated, _ = llm_augmentation_service.generate_image_and_get_label(
            ...
            validate_quality=True,  # Enable quality validation for better results
            alpha=1.0,  # CCDS exploration parameter
            exploration_mode="exploitation_only",  # Use balanced exploration/exploitation
            return_guide_images=True
        )
        ...
        new_model_path = model_service.train_model(
            ...
            updated_model_path=f"{scenario_name}_run_{run_number}_iter_{i + 1}.pth"
        )
    ...
    results_filename = RESULTS_DIR / f"{scenario_name}_run_{run_number}_results.json"
...
if __name__ == "__main__":
    ...
    for run in range(1, NUM_RUNS + 1):
        for name, params in scenarios.items():
            run_scenario(name, params, run)
    
    logging.info("--- All runs for all scenarios complete ---")